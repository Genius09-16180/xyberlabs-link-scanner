...
import { LayoutDashboard, RotateCcw, AlertTriangle } from 'lucide-react'
...
function saveLinkToLocalStorage(data) {
  try {
    ...
    const newLink = {
      ...
      category: mapRisk(data.report?.riskScore || 0),
      label: '', tags: [], connections: [],
...
function Orbs() {
  return (
    <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', overflow: 'hidden', zIndex: 0 }}>
      <div style={{ position: 'absolute', width: '700px', height: '700px', top: '-150px', right: '-150px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(0,229,255,.06) 0%, transparent 70%)', animation: 'orbDrift 18s ease-in-out infinite' }} />
      <div style={{ position: 'absolute', width: '600px', height: '600px', bottom: '-100px', left: '-100px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(155,95,255,.04) 0%, transparent 70%)', animation: 'orbDrift 22s ease-in-out 3s infinite reverse' }} />
      <style>{`@keyframes orbDrift{0%,100%{transform:translate(0,0)}33%{transform:translate(40px,-30px)}66%{transform:translate(-20px,40px)}}`}</style>
    </div>
  )
}
...
export default function ScannerPage() {
  ...
  const handleAnalyze = useCallback(async (url) => {
    setScanUrl(url); setState('scanning'); setErrorMsg(''); setAnalysisData(null); setChatOpen(false)
    try {
      const data = await analyzeUrl(url)
      setAnalysisData(data); setState('done')
            ...
            {state === 'idle' && (
              <motion.div key="hero" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -20 }}
                ...
                <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.7 }}
                  style={{ fontFamily: 'alfarn-2, sans-serif', fontWeight: 900, textTransform: 'uppercase', lineHeight: 0.9, marginBottom: '24px' }}>
                  <span style={{ display: 'block', color: 'rgba(255,255,255,.25)', fontSize: 'clamp(13px,1.8vw,18px)', fontFamily: 'JetBrains Mono, monospace', fontWeight: 400, letterSpacing: '0.14em', marginBottom: '14px' }}>XyberLabs presents</span>
                  <span style={{ display: 'block', color: 'white', fontSize: 'clamp(40px,7.5vw,78px)' }}>Link</span>
                  <span style={{ display: 'block', fontSize: 'clamp(40px,7.5vw,78px)', background: 'linear-gradient(90deg,#00E5FF,#00BFFF)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', filter: 'drop-shadow(0 0 28px rgba(0,229,255,.4))' }}>Scanner AI</span>
                </motion.h1>

                <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
                  style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '13px', color: 'rgba(255,255,255,.35)', maxWidth: '500px', margin: '0 auto 48px', lineHeight: 1.7 }}>
                  Analisi sicurezza AI su qualsiasi URL. Report dinamico, chat contestuale, board interattiva.
                </motion.p>
          ...
          <AnimatePresence mode="wait">
            {state === 'scanning' && (
              <motion.div key="scanning" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ paddingTop: '20px', paddingBottom: '40px' }}>
                <div style={{ marginBottom: '40px' }}><SearchBar onAnalyze={handleAnalyze} isLoading={true} /></div>
